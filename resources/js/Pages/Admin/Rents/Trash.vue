...
export default {
    ...
    methods: {
        async loadItems({ page, itemsPerPage, sortBy }) {
            ...
            try {
                const response = await this.$axios.get("/rents/trashed", {
                    params: {
                        page,
                        itemsPerPage,
                        sortBy: sortKey,
        ...
        async confirmRestore() {
            ...
            try {
                await this.$axios.post(
                    `/rents/${this.selectedRentsId}/restore`
                );
                this.loadItems({
                    page: 1,
                ...
                toast.success("Rents restored successfully!");
            } catch (error) {
                console.error("Error restoring Rent:", error);
                toast.error("Failed to restore Rent.");
            }
        },
        async confirmDelete() {
