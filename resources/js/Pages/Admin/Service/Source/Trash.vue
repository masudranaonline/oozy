<template>
    <v-card>
        <v-card-title class="pt-5">
            <v-row>
                <v-col cols="6"><span>Source Trash List</span></v-col>
...
export default {
    ...
    data() {
        return {
            restroreDialogName:"Are you sure you want to restore this Source?",
    ...
    methods: {
        async loadItems({ page, itemsPerPage, sortBy }) {
            ...
            try {
                const response = await this.$axios.get("/mechine/sources/trashed", {
                    params: {
                        page,
                        itemsPerPage,
                        sortBy: sortKey,
        ...
        async confirmRestore() {
            ...
            try {
                await this.$axios.post(
                    `mechine/sources/${this.selectedSourceId}/restore`
                );
                this.loadItems({
                    page: 1,
        ...
        async confirmDelete() {
            ...
            try {
                await this.$axios.delete(
                    `mechine/sources/${this.selectedSourceId}/force-delete`
                );
                this.loadItems({
                    page: 1,
                    itemsPerPage: this.itemsPerPage,
