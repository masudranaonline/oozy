<template>
    <v-card>
        <v-card-title class="pt-5">
            <v-row>
                <v-col cols="6"><span>Unit Trash List</span></v-col>
...
export default {
    ...
    data() {
        return {
            restoreDialogName: "Are you sure you want to restore this Unit ?",
    ...
    methods: {
        async loadItems({ page, itemsPerPage, sortBy }) {
            ...
            try {
                const response = await this.$axios.get("/units/trashed", {
                    params: {
                        page,
                        itemsPerPage,
                        sortBy: sortKey,
        ...
        async confirmRestore() {
            ...
            try {
                await this.$axios.post(`/units/${this.selectedUnitId}/restore`);
                this.loadItems({
                    page: 1,
        ...
        async confirmDelete() {
            ...
            try {
                await this.$axios.delete(
                    `/units/${this.selectedUnitId}/force-delete`
                );
                this.loadItems({
                    page: 1,
                    itemsPerPage: this.itemsPerPage,
